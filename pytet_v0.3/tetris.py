...
class Tetris():
    ...
    def accept(self, key): # To be implemented!!
        ...
        if self.key == 'a': # move left
            self.left -= 1
        elif self.key == 'd': # move right
            self.left += 1
        elif self.key == 's': # move down
            self.top += 1
        elif self.key == 'w': # rotate the block clockwise
            i=(i+1)%4
            self.currBlk=Matrix(arrayBlk[h][i])
        elif self.key == ' ': # drop the block
        ...
        if self.tempBlk.anyGreaterThan(1):
            if self.key == 'a': # undo: move right
                self.left += 1
            elif self.key == 'd': # undo: move left
                self.left -= 1
            elif self.key == 's': # undo: move up
                self.top -= 1
                self.state = TetrisState.newBlock
            elif self.key == 'w': # undo: rotate the block counter-clockwise
                self.idxBlockDegree = (self.idxBlockDegree - 1) % Tetris.nBlockDegree
            elif self.key == ' ': # undo: move up
                self.top -= 1 
                self.state = TetrisState.NewBlock
